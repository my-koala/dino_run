...
const SPAWNER_GROUND_COOLDOWN_MIN: int = 256
const SPAWNER_GROUND_COOLDOWN_MAX: int = 512
const SPAWNER_SKY_COOLDOWN_MIN: int = 384
const SPAWNER_SKY_COOLDOWN_MAX: int = 450
const SPAWNER_SKY_SCROLL_THRESHOLD: int = 3000
...
@onready var _display_sand: Parallax2D = $display/sand as Parallax2D
@onready var _display_clouds: Parallax2D = $display/clouds as Parallax2D
@onready var _display_dunes: Parallax2D = $display/dunes as Parallax2D
...
func _update_display(scroll: int, scroll_frac: float) -> void:
	...
	var ground_size: int = int(_display_sand.repeat_size.x / _display_sand.scroll_scale.x)
	_display_sand.screen_offset.x = fmod(float(scroll % ground_size) + scroll_frac, ground_size)
	var display_dunes_size: int = int(_display_dunes.repeat_size.x / _display_dunes.scroll_scale.x)
	_display_dunes.screen_offset.x = fmod(float(scroll % display_dunes_size) + scroll_frac, display_dunes_size)
...
func _update_spawner(scroll: int, scroll_frac: float) -> void:
	...
	while _spawn_ground < spawn_bound_r:
		var random_obstacle: int = posmod(hash(_fnl.get_noise_1d(_spawn_ground)), OBSTACLE_A.size())
		var obstacle: ObstacleA = OBSTACLE_A[random_obstacle].instantiate() as ObstacleA
